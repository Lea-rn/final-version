  ...
  for (const row of rows) {
    ...
    if (!isH && !isF && !gouvernorat && !delegation) {
      ...
      if (consecutiveEmptyRows >= EMPTY_ROW_LIMIT) {
        console.log(`[Import] Early exit triggered at row ${HEADER_ROW + 1 + skippedCount + insertedCount}. Potential end of data.`);
        break;
      }
      continue;
    }

    // Reset the counter if we find a valid row
